...
// Tells if physics engine should simulate body rotation
/**
 * @return True if the physics engine should not affect rotation and false otherwise
 */
bool RigidBody::isRotationFreezed() const {
    return mWorld.mRigidBodyComponents.getIsRotationFreezed(mEntity);
}

// Enables or disables rotation of body by physics engine simulation
/**
 *  @param isFreezed True if physics engine should not affect rotation and false otherwise
 * */
void RigidBody::setIsRotationFreezed(bool isFreezed) {

    mWorld.mRigidBodyComponents.setIsRotationFreezed(mEntity, isFreezed);

    RP3D_LOG(mWorld.mConfig.worldName, Logger::Level::Information, Logger::Category::Body,
            "Body " + std::to_string(mEntity.id) + ": Set isRotationFreezed=" + std::to_string(isFreezed),  __FILE__, __LINE__);
}
